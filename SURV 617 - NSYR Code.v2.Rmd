...
wave2$RELIG62_bl <- wave1$PIMPREL[match(wave2$IDS, wave1$IDS)]
wave3$RELIG62_bl <- wave1$PIMPREL[match(wave3$IDS, wave1$IDS)]
wave4$RELIG62_bl <- wave1$PIMPREL[match(wave4$IDS, wave1$IDS)]
```
...
```{r }
load("~/Desktop/JPSM - Masters Program/JPSM Classes/03_Fall 2024/SURV 617 - Statistical Applications/Group Project/NSYR data/Combined Data_clean.RData")

load("~/Desktop/JPSM - Masters Program/JPSM Classes/03_Fall 2024/SURV 617 - Statistical Applications/Group Project/NSYR data/Combined Data_clean_attreg1_all.RData")

load("~/Desktop/JPSM - Masters Program/JPSM Classes/03_Fall 2024/SURV 617 - Statistical Applications/Group Project/NSYR data/Combined Data_clean_attreg1_234.RData")
```
...
#Less than 0.05 = no significant difference in models?
lrt.pvalue(random_slope_int, random_int)
```

##d. How do demographics impact attendance (ATTREG)?
``` {r}
demos <- glmer(ATTREG ~ year + RELTRAD_bl + TEENRACE_bl + 
                 TEENSEX_bl + AGECATS_bl + (1|IDS), 
               family=binomial, 
               #nAGQ = 5, #slows down time to calculate
               data=waves)
summary(demos)
#Fixed effects not sig = black protestant, morman, asian, islander, native american, race other, age?!?!?
```

###d2. Add variation with year?
```{r }
demos2 <- glmer(ATTREG ~ year + RELTRAD_bl + TEENRACE_bl + 
                 TEENSEX_bl + AGECATS_bl + 
                  (1 + year |IDS), 
               family=binomial, 
               data=waves)
summary(demos2) #many more become non-significant
```

###d. Compare
```{r }
#Demos2 (slope variation by year) has lower AIC and BIC
#pvalue = <0.05 --> demos2 preferred
anova(demos, demos2)

# it = 1???
lrt.pvalue(demos, demos2)
```

##e. What about religious predictors?
```{r }
relfact <- glmer(ATTREG ~ year + PIMPREL_bl + RELTRAD_bl + 
                   FAITH1 + PRAYALON_bl + GRACE_bl +
                   READBIBL_bl + (1|IDS), 
                 family=binomial, 
                 data=waves)
...
###e2. Add variation by year?
```{r }
relfact2 <- glmer(ATTREG ~ year + PIMPREL_bl + RELTRAD_bl + 
                   FAITH1 + PRAYALON_bl + GRACE_bl +
                   READBIBL_bl + (1 + year | IDS), 
                 family=binomial, 
                 data=waves)
summary(relfact2) #most religious affiliation not sig
```

###e. Compare
```{r }
#relfact has lower AIC and BIC
#pvalue = >0.05 --> relfact preferred (without variation by year)
anova(relfact, relfact2)

# it = 1???
lrt.pvalue(relfact, relfact2)
```
...
final_mod <- glmer (ATTREG ~ year + 
                      #demographics
                      RELTRAD_bl + TEENRACE_bl_white + TEENSEX_bl + AGECATS_bl +
                      #religion practice vars
                      PIMPREL_bl + FAITH1 + GRACE_bl + READBIBL_bl +
                      (1 + year| IDS),
                    family=binomial,
                    data=waves) 
...
###z2. With RELIG62 instead of RELTRAD
```{r }
#tabyl(waves$RELIG62_bl)
#tabyl(waves$RELIG62_bl_r)
#tabyl(waves$FAITH1)

final_mod_rel62 <- glmer (ATTREG ~ year + 
                      #demographics
                      RELIG62_bl_r + TEENRACE_bl_white + TEENSEX_bl + AGECATS_bl +
                      #religion practice vars
                      PIMPREL_bl + FAITH1 + GRACE_bl + READBIBL_bl +
                      (1 + year| IDS),
                    family=binomial,
                    data=waves) 
summary(final_mod_rel62) #more vars are significant
#the baseline age now is significant
```

###z. Compare
```{r }
#doesn't work due to different sized datasets
#anova(final_mod, final_mod_rel62)

#final_mod has lower AIC and BIC
#final_mod has less significcant predictors - due to fewer REL catagroes?

# it = 0???
lrt.pvalue(final_mod, final_mod_rel62)
```
...
##d. How do demographics impact attendance (ATTREG)?
``` {r}
demos_attreg <- glmer(ATTREG ~ year + RELTRAD_bl + TEENRACE_bl + 
                 TEENSEX_bl + AGECATS_bl + (1|IDS), 
               family=binomial, 
               #nAGQ = 5, #slows down time to calculate
               data=waves_attreg)
summary(demos_attreg)
#Fixed effects not sig = black protestant, morman, asian, islander, native american, race mixed, race other, age?!?!?
```

###d2. Add variation with year?
```{r }
demos2_attreg <- glmer(ATTREG ~ year + RELTRAD_bl + TEENRACE_bl + 
                 TEENSEX_bl + AGECATS_bl + 
                  (1 + year |IDS), 
               family=binomial, 
               data=waves_attreg)
summary(demos2_attreg) #many more become non-significant
```

###d. Compare
```{r }
#Demos2 (slope variation by year) has lower AIC and BIC
#pvalue = <0.05 --> demos2_attreg preferred
anova(demos_attreg, demos2_attreg)

# it = 1???
lrt.pvalue(demos_attreg, demos2_attreg)
```

##e. What about religious predictors?
```{r }
relfact_attreg <- glmer(ATTREG ~ year + PIMPREL_bl + RELTRAD_bl + 
                   FAITH1 + PRAYALON_bl + GRACE_bl +
                   READBIBL_bl + (1|IDS), 
                 family=binomial, 
                 data=waves_attreg)
...
###e2. Add variation by year?
```{r }
relfact2_attreg <- glmer(ATTREG ~ year + PIMPREL_bl + RELTRAD_bl + 
                   FAITH1 + PRAYALON_bl + GRACE_bl +
                   READBIBL_bl + (1 + year | IDS), 
                 family=binomial, 
                 data=waves_attreg)
summary(relfact2_attreg) #most religious affiliation not sig
```

###e. Compare
```{r }
#relfact has lower AIC and BIC
#pvalue = >0.05 --> relfact2_attreg preferred (without variation by year)
anova(relfact_attreg, relfact2_attreg)

# it = 1???
lrt.pvalue(relfact_attreg, relfact2_attreg)
```
...
final_mod_rel62_attreg <- glmer (ATTREG ~ year + 
                      #demographics
                      RELIG62_bl_r + TEENRACE_bl_white + TEENSEX_bl + AGECATS_bl +
                      #religion practice vars
                      PIMPREL_bl + FAITH1 + GRACE_bl + READBIBL_bl +
                    ...
                    family=binomial,
                    data=waves_attreg) 
summary(final_mod_rel62_attreg) #similar results?
```

###z. Compare
```{r }
#doesn't work due to different sized datasets
#anova(final_mod_attreg, final_mod_rel62_attreg)

#REL62 model had lower AIC and BIC

# it = 0???
lrt.pvalue(final_mod_attreg, final_mod_rel62_attreg)
```

###z. Examine Reduced Model
```{r }
#summary(final_mod_rel62_attreg)

final_mod_rel62_attreg_reduced <- glmer (ATTREG ~ year + 
                      #demographics
                      RELIG62_bl_r + TEENRACE_bl_white + TEENSEX_bl + AGECATS_bl +
                      #religion practice vars
                      #PIMPREL_bl + 
                        FAITH1 + GRACE_bl + READBIBL_bl +
                      (1 + year| IDS),
                    family=binomial,
                    data=waves_attreg) 
summary(final_mod_rel62_attreg_reduced) #similar results?
```
...
```{r }
#kitchen sink
glmer <- glmer(ATTREG ~ PIMPREL_bl + RELIG62_bl + RELTRAD_bl + FAITH1 + PRAYALON_bl + GRACE_bl + READBIBL_bl + TEENRACE_bl + TEENSEX_bl + AGECATS_bl + (1 + year | IDS),
             family = binomial,
             #nARG = 7,
             data = waves)
summary(glmer)
