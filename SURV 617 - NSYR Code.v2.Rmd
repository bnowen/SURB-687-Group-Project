...
```{r }
load("~/Downloads/Combined Data_clean_updated.RData")
...
marginal_demos <- geeglm(ATTREG ~ year + RELIG62_bl_r + year*RELIG62_bl_r +
                           TEENRACE_bl_white + year*TEENRACE_bl_white + 
                 TEENSEX_bl + year*TEENSEX_bl + AGECATS_bl + year*AGECATS_bl,
                 data=data,
                    id=IDS,
                 family=binomial("logit"),
                 corstr = "exchangeable",
                 na.action=na.omit)
summary(marginal_demos)

marginal_relig <- geeglm(ATTREG ~ year + PIMPREL_bl_r + 
...
subsample <- sample_n(waves, 100)
table(subsample$year)
save(subsample, file = "~/Downloads/ATTREG_100_cases.RData")
...
wavesdata$year[wavesdata$year==0] <- 1
wavesdata$year[wavesdata$year==2] <- 2
wavesdata$year[wavesdata$year==4] <- 3
wavesdata$year[wavesdata$year==10] <- 4

marginal <- geeglm(ATTREG ~ year + RELIG62_bl,
                   data=waves,
                    id=IDS,
                 family=binomial("logit"),
                 corstr = "exchangeable",
...
model <- glm(ATTREG ~ year, data=wavesdata, family=binomial("logit"))
```